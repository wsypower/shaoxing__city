...
!(function(This) {
  ...
  function mergeData(data, newData, replace) {
    for (var key in newData)
      replace
        ...
        : (data[key] = void 0 !== data[key] ? data[key] : newData[key]);
  }
  ...
  function callHandlerEx(handlerName, data) {
    ...
    var responseCallback = function(responseData) {
        ...
        '0' === errorCode
          ...
          : '-1' === errorCode
          ? onCancel && onCancel.call(null, result)
          : onFail && onFail.call(null, result, errorCode);
      },
      registered = false;
    ...
    if (bridgeModuleDict.android) {
      ...
      if (registered) {
        WebViewJavascriptBridge.registerHandler(handlerName, function(data, callback) {
  ...
  var bridgeModuleDict = {
    ...
    compareVersion: function(a, b, c) {
      ...
      for (var d, e, f = a.split('.'), g = b.split('.'); d === e && g.length > 0; )
        (d = f.shift()), (e = g.shift());
  ...
  var registerModule = function(functionName, callbackFunc) {
    for (
      var nameSplit = functionName.split('.'),
        moduleDict = bridgeModuleDict,
        index = 0,
        splitLen = nameSplit.length;
      splitLen > index;
      index++
    )
      index === splitLen - 1 && (moduleDict[nameSplit[index]] = callbackFunc),
  ...
  handleNameArray.forEach(function(handleName) {
    ...
    'object' == typeof module && module && 'object' == typeof module.exports
      ? (module.exports = bridgeModuleDict)
...
if (typeof jQuery == 'undefined') {
  ...
  if (host == 'jhsbmxcx.yyhj.zjzwfw.gov.cn') {
    ...
    document.write(
      '<script type="text/javascript" src="http://app.zjzwfw.gov.cn/client/jssdkcheck/js/jquery-1.8.3.min.js"></script>',
    );
  }
}
...
window.jmportal = {
  login: { getUserInfo: getUserInfoString },
  QRCode: { getQRCode: getQRCodeString },
};
