  ...
  $.fn.off = function(event, selector, callback) {
    return this.each(function() {
      ...
      if (!event) {
        ...
        var delegateCallbacks =
          delegates[_mid] && delegates[_mid][event] && delegates[_mid][event][selector];
        $.each(
          delegateCallbacks,
  ...
  var handle = function(event, touch) {
    ...
    switch (event.type) {
      case $.EVENT_END:
        ...
        if (
          !target ||
          (target.disabled || (target.classList && target.classList.contains('mui-disabled')))
        ) {
          return;
        }
...
(function($) {
  $.addInit({
    ...
    handle: function() {
      ...
      var hasPulldown =
        pullRefreshOptions.down && pullRefreshOptions.down.hasOwnProperty('callback');
    ...
    $.extend(
      {
        ...
        _start: function(e) {
          ...
          if (e.touches && e.touches.length && e.touches[0].clientX > 30) {
            e.target &&
              !this._preventDefaultException(e.target, this.options.preventDefaultException) &&
              e.preventDefault();
          }
          if (!this.loading) {
    ...
    _initSnap: function() {
      ...
      if (!this.currentPage.x) {
        ...
        this.lastSlideNumber =
          typeof this.lastSlideNumber === 'undefined' ? 0 : this.lastSlideNumber;
      } else {
        this.slideNumber = this._fixedSlideNumber();
        this.lastSlideNumber =
          typeof this.lastSlideNumber === 'undefined' ? this.slideNumber : this.lastSlideNumber;
      }
      this.options.startX = this.currentPage.x || 0;
    },
    _getSnapX: function(offsetLeft) {
        ...
        $.plusReady(function() {
          if (self.options.down.style == 'circle') {
            ...
            webview.addEventListener('close', function() {
              var attrWebviewId =
                self.options.webviewId && self.options.webviewId.replace(/\//g, '_'); //替换所有"/"
  ...
  var OffCanvas = $.Class.extend({
    ...
    toggle: function(directionOrOffCanvas) {
      ...
      if (directionOrOffCanvas && directionOrOffCanvas.classList) {
        direction = directionOrOffCanvas.classList.contains(CLASS_OFF_CANVAS_LEFT)
          ? 'left'
          : 'right';
...
(function($, window, document) {
    ...
    if (translateX !== lastTranslateX) {
      if (buttonsRight && buttonsRight.length > 0) {
        ...
        if (translateX < -sliderActionRightWidth) {
          translateX =
            -sliderActionRightWidth - Math.pow(-translateX - sliderActionRightWidth, overFactor);
        }
      ...
      if (buttonsLeft && buttonsLeft.length > 0) {
        ...
        if (translateX > sliderActionLeftWidth) {
          translateX =
            sliderActionLeftWidth + Math.pow(translateX - sliderActionLeftWidth, overFactor);
        }
        for (var i = 0, len = buttonsLeft.length; i < len; i++) {
          var buttonLeft = buttonsLeft[i];
          if (typeof buttonLeft._buttonOffset === 'undefined') {
            buttonLeft._buttonOffset =
              sliderActionLeftWidth - buttonLeft.offsetLeft - buttonLeft.offsetWidth;
          }
  ...
  var handleEvent = {
    ...
    dragend: function(event) {
      ...
      var action = 'close';
      var actionsWidth =
        sliderDirection === 'toLeft' ? sliderActionRightWidth : sliderActionLeftWidth;
...
(function($, window, document) {
  ...
  var createAlert = function(message, title, btnValue, callback, type) {
    ...
    if (!$.os.plus || type === 'div') {
      return createPopup(
        createInner(message, title || '提示') + createButtons([btnValue || '确定']),
        callback,
      );
    }
  ...
  var createConfirm = function(message, title, btnArray, callback, type) {
    ...
    if (!$.os.plus || type === 'div') {
      return createPopup(
        createInner(message, title || '提示') + createButtons(btnArray || ['取消', '确认']),
        callback,
      );
    }
